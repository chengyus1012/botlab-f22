...
bool MotionPlanner::isValidGoal(const mbot_lcm_msgs::pose_xyt_t& goal) const
{
    ...
    if(distances_.isCellInGrid(goalCell.x, goalCell.y))
    {
        ...
        std::cout <<" obstacle distance for goal cell: " <<distances_(goalCell.x, goalCell.y)<<std::endl;
...
void MotionPlanner::setParams(const MotionPlannerParams& params)
{
    ...
    searchParams_.maxDistanceWithCost = 2.5 * searchParams_.minDistanceToObstacle;
    searchParams_.distanceCostExponent = 0.5;
}
